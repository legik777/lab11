...
boost::log::v2s_mt_nt6::trivial::severity_level log_lvl = boost::log::trivial::error;
...
void readFileFunction(std::string input_file) {
    ...
    db->ListColumnFamilies(options, input_file, &column_families); //ñ÷èòûâàåì ñåìüè 
    ...
    status = rocksdb::DB::Open(rocksdb::DBOptions(), input_file, column_familiesD, &idb.handles, &db); //îòêðûâàåì èñõîäíîå õðàíëèùå â ðåæèìå òîëüêî äëÿ ÷òåíèÿ
...
void writeFileFunction(int n, std::string path) {
    while (write_work) {
        ...
        if (ndb.size() !=0) {
            rocksdb::Status status;
            //std::cout<< ndb.back().family<< std::endl<< ndb.back().key<< std::endl<< ndb.back().value << std::endl;
            
status = nrdb->Put(rocksdb::WriteOptions(), idb.handles[ndb.back().family], rocksdb::Slice(ndb.back().key), rocksdb::Slice(ndb.back().value));
            ndb.pop_back();
        }
        else {
...
void EApplication::menu(int argc, const char** argv) {
    m_desk.add_options()("help", "âûçîâ ñïðàâêè")
        ...
        ("thread-count", boost::program_options::value<int>(&thread_count)->composing()->default_value(std::thread::hardware_concurrency()), "êîëè÷åñòâî ïîòîêîâ")
        ("output", boost::program_options::value<std::string>(&path)->composing()->default_value("dbcs-storage.db"), "Ïóòü ê íîâîìó õðàíèëèùó")
        ("input-file", boost::program_options::value<std::string>(&input_path)->composing()->default_value("source.db"), "Ïóòü ê õðàíèëèùó");
    p.add("input-file", -1);
...
int EApplication::exec() {
    ...
    if (m_vm.count("log-level") && m_vm.count("thread-count") && m_vm.count("output") && m_vm.count("input-file")) {
        if (log_level == "info")
            log_lvl = boost::log::trivial::info;
        if (log_level == "warning")
            log_lvl = boost::log::trivial::warning;
        ...
        if (m_vm["input-file"].as<std::string>() != "source.db") {
            ...
            createFileFunction(m_vm["output"].as<std::string>());
                if (src.size() != 0) {
                    ...
                    for (int i = 0; i < thread_count; i++) {
                        write.push_back(std::thread(writeFileFunction, i, m_vm["output"].as<std::string>()));
                    }
                    for (int i = 0; i < thread_count; i++) {
                        hesh[i].join();
                    }
